...
export async function getPlaylistTracksService(userId?: string) {
    try {
        ...
        if (userId) {
            myPlaylistsUrl = `https://api.spotify.com/v1/users/${userId}/playlists?offset=0&limit=20`;
        }
        ...
        async function getMyPlaylists(url, playlistTrackArr) {
            ...
            playlistTrackArr = [
                ...playlistTrackArr,
                ...myPlaylistsResponse.data.items,
            ];
        ...
        for (let i = 0; i < playlists.length; i++) {
            ...
            async function getPlaylistTrackEntries(
                url,
                playlistTrackEntriesArr
            ) {
                const playlistTrackEntriesResponse = await axios.get(url);

                playlistTrackEntries.push(
                    ...playlistTrackEntriesResponse.data.items
                );
...
// DONE HERE
        ...
        for (let i = 0; i < numOfApiCalls; i++) {
            const rangeOfArtistIds = arrayOfArtistIds.slice(
                i * numOfArtistsPerReq,
                (i + 1) * numOfArtistsPerReq
            );
            ...
            artistRepsonse.data.artists.forEach((artist) => {
                ...
                playlistArtistsMap.get(artist.id).popularity =
                    artist.popularity;
            });
        }
        ...
        for (const trackData of playlistTracksMap.values()) {
            ...
            if (playlistArtistsMap.has(primaryArtist.id)) {
                trackData.genres = playlistArtistsMap.get(
                    primaryArtist.id
                ).genres;
            }
        }
