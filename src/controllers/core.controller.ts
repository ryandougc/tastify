...
export const getMyDetails = async (req, res, next) => {
        try {
                const Profile = mongoose.model('Profile', profileSchema, req.session.userId)
...
export const analyzeMyData = async (req, res, next) => {
        try {
                console.time()
                const currentUserTracks = await getCurrentUserTracksService(req.session.access_token)

                const currentUserGenreBreakdown = await createGenreBreakdownService(currentUserTracks)

                await saveGenreBreakdownService(currentUserGenreBreakdown, req.session.userId)
...
export const analyzePublicUserData = async (req, res, next) => {
        try {
                ...
                const publicUserTracks = await getPublicUserTracksService(req.session.access_token, userId)

                const publicUserGenreBreakdown = await createGenreBreakdownService(publicUserTracks)
...
export const getMyAnalysis = async (req, res, next) => {
        try {
                ...
                const currentUserGenreBreakdown = await getUserDataFromDbService(userId)

                const currentUserTopTenGenres = await getTopTenService(currentUserGenreBreakdown)
...
async (req, res, next) => {
        try {
                ...
                if(await checkUserExistsInDb(publicUserId)) {
                        publicUserGenreBreakdown = getUserDataFromDbService(publicUserId)
                } else {
                        const publicUserTracks = await getPublicUserTracksService(req.session.access_token, publicUserId)

                        publicUserGenreBreakdown = await createGenreBreakdownService(publicUserTracks)
                }
                ...
                if(await checkUserExistsInDb(currentUserId)) {
                        currentUserGenreBreakdown = await getUserDataFromDbService(currentUserId)
                } else {
                        const currentUserTracks = await getCurrentUserTracksService(req.session.access_token)

                        currentUserGenreBreakdown = await createGenreBreakdownService(currentUserTracks)
        
                        await saveGenreBreakdownService(currentUserGenreBreakdown, req.session.userId)
                }

                const publicUserTopTenGenres = await getTopTenService(publicUserGenreBreakdown)
                const currentUserTopTenGenres = await getTopTenService(currentUserGenreBreakdown)

                res.status(200).send({publicUserTopTenGenres, currentUserTopTenGenres})
        } catch(err) {
                console.log(err)
        ...
        try {
                ...
                for (const trackData of topTracksMap.values()) {
                        ...
                        if(topTracksArtistsMap.has(primaryArtist.id)) {
                                trackData.genres = topTracksArtistsMap.get(primaryArtist.id).genres
                        }
                }
